function fit_de_delay_vp_np(SbjIdx,DelayIdx,NIdx,TestIdx)
...
%     FileName = [PathName 'de_delay_vp_test_' num2str(SbjIdx) '.mat'];
    FileName = [PathName 'de_delay_vp_np_test_' num2str(SbjIdx)];
...
%     FileName = [PathName 'de_delay_vp_' SbjName '_' num2str(DelayIdx*1000) '.mat'];
    FileName = [PathName 'de_delay_vp_np_' SbjName '_' num2str(DelayIdx*1000) '_' num2str(NIdx)];
...
FitnessFun = {@LLH,TestIdx,SbjData,SampleSize};
...
DataModel = create_fakedata_DE_np(ntrials, JbarOut, tauOut);

% model fitting
generate_modelfit_DE_VP(TestIdx,SbjName,FileName,SbjData,DelayIdx,NIdx,DataModel,ParamInVec,ParamOutVec,fval);

function LLH_ga_VP = LLH(ParamVec,TestIdx,SbjData,SampleSize)
...
function generate_modelfit_DE_VP(TestIdx,SbjName,FileName,SbjData,DelayIdx,NIdx,DataModel,ParamInVec,ParamOutVec,fval);
