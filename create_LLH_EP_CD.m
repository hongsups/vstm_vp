function [L, ML_estimate, BIC] = create_LLH_EP_CD(capacity_idx,expid,subjid)

% load parameters from the table file
BT_path = 'CD/bigtable/';
files = dir([BT_path 'CD_T_EP_1.mat']);
load([BT_path files.name],'nsteps','J_EP_vec','power_vec','p_change_vec','magstep','mag','sampleSize_n','Nvec');
...
%p_response_yes -> (ip, ipc, ic, i, j)

% compute likelihood
% initialize
if capacity_idx == 1
    p_L = zeros(length(J_EP_vec),length(power_vec),length(p_change_vec));
else
    p_L = zeros(length(J_EP_vec),length(power_vec),length(p_change_vec),length(capacity_vec));
end

for ij = 1:length(J_EP_vec)
    % load a lookup table
    files = dir([BT_path 'CD_T_EP_' num2str(ij) '.mat']);
    ...
    for ipc = 1:length(p_change_vec)
        for ip = 1:length(power_vec)
            if capacity_idx == 1
                ...
                p_L(ij,ip,ipc) = sum(p_log_like_mat(:));
            elseif capacity_idx == 2    % fixed K
                for ic = 1:length(capacity)
                    ...
                    p_log_like_response_no = response_no_trials.*log(p_no_response);
                    p_log_like_mat = p_log_like_response_yes + p_log_like_response_no;
                    p_L(ij,ip,ipc,ic) = sum(p_log_like_mat(:));
                end
...
if capacity_idx == 1
%     L = log(sum(expL(:))) + p_L_max - log(length(J_EP_vec)*length(power_vec)*length(p_change_vec));
    L = log(trapz(J_EP_vec,trapz(power_vec,trapz(p_change_vec,expL,3),2),1)) + p_L_max - log(range(J_EP_vec)*range(power_vec)*range(p_change_vec));
else
%     L = log(sum(expL(:))) + p_L_max - log(length(J_EP_vec)*length(power_vec)*length(p_change_vec)*length(capacity_vec));
    L = log(trapz(J_EP_vec,trapz(power_vec,trapz(p_change_vec,trapz(capacity_vec,expL,4),3),2),1)) + p_L_max - log(range(J_EP_vec)*range(power_vec)*range(p_change_vec)*range(capacity_vec));
end
...
[M, I] = max(p_L(:));     
if capacity_idx == 1
    [idx_j, idx_p, idx_pc] = ind2sub(size(p_L),I);
    ML_estimate = [J_EP_vec(idx_j) power_vec(idx_p) p_change_vec(idx_pc) 8];
elseif capacity_idx > 1
    [idx_j, idx_p, idx_pc, idx_c] = ind2sub(size(p_L),I);
    ML_estimate = [J_EP_vec(idx_j) power_vec(idx_p) p_change_vec(idx_pc) capacity_vec(idx_c)];
end
